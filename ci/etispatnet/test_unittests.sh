...
source ./ci/etispatnet/matrix.sh
...
cd build-ci/etispatnet-$BUILD_TARGET
...
if [ "$DIRECT_WINE_EXEC_TESTS" = "true" ]; then
  ...
  wine ./src/test/test_etispatnet.exe
else
  if [ "$RUN_UNIT_TESTS_SEQUENTIAL" = "true" ]; then
    ./src/test/test_etispatnet --catch_system_errors=no -l test_suite
  else
