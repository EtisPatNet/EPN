...
The EtisPatNet Core daemon can be configured to act as a trusted "border
router", implementing the etispatnet wire protocol and relay, making
...
The ZeroMQ feature in EtisPatNet Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building etispatnetd:
...
For instance:
    ...
    $ etispatnetd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubrawtx=ipc:///tmp/etispatnetd.tx.raw \
...
These options can also be provided in etispatnet.conf.
...
From the perspective of etispatnetd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into etispatnetd directly. Furthermore, no information is
...
using. EtisPatNetd appends an up-counting sequence number to each
