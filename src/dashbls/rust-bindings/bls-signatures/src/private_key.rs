...
impl PrivateKey {
    ...
    pub fn generate_etispatnet(rng: &mut StdRng) -> Result<Self, BlsError> {
    ...
    #[cfg(feature = "etispatnet_helpers")]
    pub fn generate_etispatnet_many(count: usize, rng: &mut StdRng) -> Result<Vec<Self>, BlsError> {
        (0..count)
            .into_iter()
            .map(|_| Self::generate_etispatnet(rng))
            .collect()
    }
