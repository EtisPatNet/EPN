...
const char * const BITCOIN_CONF_FILENAME = "etispatnet.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //etispatnet-tx using stdin
...
{
    // Windows: C:\Users\Username\AppData\Roaming\EtisPatNet
    // macOS: ~/Library/Application Support/EtisPatNet
    // Unix-like: ~/.etispatnet
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "EtisPatNet";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/EtisPatNet";
#else
    // Unix-like
    return pathRet / ".etispatnet";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty etispatnet.conf if it does not exist
